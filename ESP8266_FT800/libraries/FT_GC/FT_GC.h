...
class FT_GC : public FT_Trans
{
	...
	FT_GEStatus WriteCmdfromflash( const byte *Src,uint32_t NBytes);	
	...
	FT_GEStatus TransferCmdfromflash(const byte *Src,uint32_t NBytes);	
...
FT_GEStatus FT_GC<FT_Trans>::TransferCmd(uint8_t *Src,uint32_t NBytes)
{
	...
	while(NBytes)
	{
		...
		if(Count > FreeSpace)
		{
			...
			//get the free space
			NBytes -= Count;
			Count = _min(NBytes,FT_CMDFIFO_SIZE/2);//atleast wait for half the buffer completion
...
template<class FT_Trans>
FT_GEStatus FT_GC<FT_Trans>::TransferCmdfromflash( const byte *Src,uint32_t NBytes)
{
	uint32_t i,Count;
	//align the NBytes to multiple of 4
	NBytes = (NBytes + 3)&(~3);
...
FT_GEStatus FT_GC<FT_Trans>::WriteCmdfromflash( const byte *Src,uint32_t NBytes)
{
	FT_GEStatus Status;
	if(0 == TrnsFlag)
	{
